    ...
    public sealed class Decision
    {
        ...
        public void SetChances(double dropChance, double tamperChance, double duplicateChance, double reorderChance)
        {
            ...
            if (drop > 0)
            {
                thresholds.Add(new()
                {
                    packetAction = PacketAction.Drop, threshold = drop
                });
            }
            ...
            if (tamper > 0)
            {
                thresholds.Add(new()
                {
                    packetAction = PacketAction.Tamper, threshold = drop + tamper
                });
            }
            ...
            if (duplicate > 0)
            {
                thresholds.Add(new()
                {
                    packetAction = PacketAction.Duplicate, threshold = drop + tamper + duplicate
                });
            }
            ...
            thresholds.Add(new()
            {
                packetAction = PacketAction.Normal, threshold = PartsPerTenThousand.Divisor
            });
        }

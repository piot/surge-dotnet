...
namespace Piot.Surge.Pulse.Host
{
    public sealed class SnapshotSyncerClient
    {
        ...
        public Dictionary<uint, EntityId> AssignedPredictedEntityForLocalPlayers { get; } = new();

        public uint[] AssignedPredictedEntityIdValuesForLocalPlayers => AssignedPredictedEntityForLocalPlayers.Select(x => (uint)x.Value.Value).ToArray();
        ...
        public void SetAssignedPredictedEntity(LocalPlayerIndex localPlayerIndex, EntityId entity)
        {
            if (entity.Value == 0)
            {
